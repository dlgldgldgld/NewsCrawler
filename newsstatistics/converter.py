...
class Converter() :
    def __init__(self , inputfile_path : str, inputfile_format :str , outfile_path : str, outfile_name : str = None ) : 
    ...
    def __fileopen( self ) :
        ...
        try : 
            self.__origin_data_frame = self.__spark.read.format( file_format )\
    ...
    def __remove_duprecords(self ):
        ...
        group_inter_comm = self.__origin_data_frame.groupBy(['link']).agg(max("interest_cnt").alias("interest_cnt"), max("comm_cnt").alias("comm_cnt"))
        res = base_col.join(group_inter_comm, 'link', 'inner').withColumn("date", to_date(current_date() , "yyyy-MM-dd"))
        return res
...
if __name__ == "__main__" :
    if len(sys.argv) < 4 :
        logging.error("This program needs 4 input parameters. [1] = inputfile_path, [2] = inputfile_format [3] = outfile_path, [4] = outfile_name")
        exit(0)
    ...
    t = Converter( inputfile_path = inputfile_path, inputfile_format = inputfile_format, outfile_path= outfile_path, outfile_name = outfile_name )
    t.exec( )